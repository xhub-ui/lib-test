--[[
    NetroUI - UI Library 
    Status: Revised for Custom Marketing Config
]]
...
Netro65UI.Version = "5.1.0" -- Version bump for Bubble Update
...
--// Locking System (Optimized & Non-Stacking)
function Utility:LockElement(instance, text, reasonType)
    ...
    if existingOverlay then
        ...
        local icon = existingOverlay:FindFirstChild("LockIcon")
        local lbl = existingOverlay:FindFirstChild("LockLabel")
        
        if icon then icon.ImageColor3 = lockColor end
        if lbl then 
            lbl.Text = text or "Locked"
            lbl.TextColor3 = lockColor 
        end
        return -- Exit function, work done
    end
...
function Netro65UI:CreateWindow(props)
    ...
    --// Marketing Config //
    -- Extract custom settings or use default
    ...
    local bubbleConfig = props.Bubble or {
        Type = "Image", -- "Text", "Emoji", "Image"
        Content = "rbxassetid://13463914830", -- Default Icon
    ...
    local BubbleButton = Utility:Create("TextButton", {
        ...
        Position = UDim2.new(0.1, 0, 0.1, 0), -- Default spawn
        ...
        Utility:Create("UICorner", {CornerRadius = UDim.new(0, 14)}), -- Rounded Box
        Utility:Create("UIStroke", {Color = Netro65UI.Theme.Outline, Thickness = 1.5})
    })
    
    -- Add content to Bubble Button based on Type
    local BubbleContent
    
    if bubbleConfig.Type == "Image" then
        BubbleButton.BackgroundTransparency = 1 -- Hide box for pure icon look if wanted, or keep 0
        BubbleButton.BackgroundColor3 = Color3.new(1,1,1) -- Reset if image
        ...
        BubbleContent = Utility:Create("ImageLabel", {
            ...
            ImageColor3 = Color3.new(1,1,1) -- Original image color
        })
    elseif bubbleConfig.Type == "Text" or bubbleConfig.Type == "Emoji" then
        ...
        BubbleContent = Utility:Create("TextLabel", {
            ...
            TextColor3 = Netro65UI.Theme.Text,
            Font = Netro65UI.Theme.FontBold,
            TextSize = (bubbleConfig.Type == "Emoji") and 28 or 20,
            TextWrapped = true
        })
    end
    ...
    local function ToggleWindow(showWindow)
        ...
        if showWindow then
            ...
            -- Hide Main Window -> Show Bubble
            BubbleButton.Visible = true
            -- Optional: Animation before hiding
    ...
    -- Close button logic: Destroys everything
    CloseBtn.MouseButton1Click:Connect(function() 
        Acrylic:Disable() 
    ...
    -- Minimize button logic: Hides UI -> Shows Bubble
    ...
    local NavContainer = Utility:Create("ScrollingFrame", {
        ...
        BackgroundTransparency = 1, 
        Size = UDim2.new(0, NavWidth, 1, -75), -- Adjusted height to make room for profile card
        Position = UDim2.new(0, 0, 0, 10), 
    ...
    -- Profile Card (Adjusted Position)
    local ProfileCard = Utility:Create("Frame", {
        Parent = ContentContainer, 
        BackgroundColor3 = Netro65UI.Theme.Secondary, 
        Size = UDim2.new(0, NavWidth - 20, 0, 45), 
        Position = UDim2.new(0, 10, 1, -60), -- Adjusted from -50 to -60 to prevent clipping/overlap
        BorderSizePixel = 0, 
    ...
    --// Modified AddTab to accept LayoutOrder
    function WindowObj:AddTab(name, layoutOrder)
        ...
        local TabBtn = Utility:Create("TextButton", {
            ...
            TextXAlignment = Enum.TextXAlignment.Left,
            TextTruncate = Enum.TextTruncate.AtEnd,
            LayoutOrder = currentOrder -- Assign LayoutOrder here
        }, {
        ...
        -- Special access to Page for Players Info Gen
        Tab.PageInstance = Page

        function Tab:AddSection(sectionName)
            local Section = {}
            -- Adjusted spacing for section header
            local SectionFrame = Utility:Create("Frame", {
                ...
                Utility:Create("TextLabel", {
                    ...
                    TextColor3 = Netro65UI.Theme.Accent, 
                    BackgroundTransparency = 1, 
                    Size = UDim2.new(1, -20, 0, 35), -- Increased height to 35 for better spacing
                ...
                Utility:Create("Frame", {
                    Name = "Container", 
                    BackgroundTransparency = 1, 
                    Position = UDim2.new(0, 0, 0, 40), -- Moved container down to 40
                    Size = UDim2.new(1, 0, 0, 0)
                }, {
                    Utility:Create("UIListLayout", {
                        SortOrder = Enum.SortOrder.LayoutOrder, 
            ...
            ItemContainer.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
                SectionFrame.Size = UDim2.new(
                    0.96, 0, 
                    0, ItemContainer.UIListLayout.AbsoluteContentSize.Y + 55 -- Adjusted frame calculation
                ) 
            end)
    ...
    -- Function to populate Players Info tab
    local function GeneratePlayerInfo(parentPage)
        ...
        local UpgradeBtn = Utility:Create("TextButton", {
            ...
            BackgroundColor3 = Netro65UI.Theme.Accent,
            Text = finalUpgradeText, -- Uses dynamic variable
            TextColor3 = Color3.new(1,1,1),
            Font = Netro65UI.Theme.FontBold,
            TextSize = 12
        }, {
        ...
        UpgradeBtn.MouseButton1Click:Connect(function()
            if setclipboard then
                setclipboard(finalUpgradeLink) -- Uses dynamic variable
                Netro65UI:Notify({Title="VIP Upgrade", Content="Purchase link copied!", Type="Success", Duration=3})
                UpgradeBtn.Text = "Link Copied!"
                task.wait(1)
                UpgradeBtn.Text = finalUpgradeText -- Resets to dynamic variable
            end
        end)
    end
    
    -- Create the Players Info Tab automatically (Always first due to LayoutOrder -9999)
    local PlayersInfoTab = WindowObj:AddTab("Players Info", -9999)
    GeneratePlayerInfo(PlayersInfoTab.PageInstance)

    --// AUTO-GENERATE SETTINGS TAB (Placed at the bottom logic)
    -- Using a very high LayoutOrder (99999) ensures it stays at the bottom
    ...
    -- Removed Universal Config Toggle & Auto-Save Toggle
    ...
    -- UI Settings Section
    local UISettings = SettingsTab:AddSection("UI Effects")
    ...
    -- Register default keybind internally without UI section
