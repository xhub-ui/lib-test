--[[
    NetroUI - UI Library 
    Status: Updated (v5.3.0)
    Updates: Cooldown Button, Bento/Dual Column Layout, Side Setting
]]
...
function Netro65UI:CreateWindow(props)
    local WindowObj = {}
    props = props or {}
    local windowWidth = props.Width or 600 -- Increased width for Dual Column
    ...
    function WindowObj:AddTab(name, layoutOrder)
        ...
        -- Logic to update CanvasSize based on taller column
        local function UpdateCanvas()
            local leftH = LeftColumn.UIListLayout.AbsoluteContentSize.Y
            local rightH = RightColumn.UIListLayout.AbsoluteContentSize.Y
            local maxH = math.max(leftH, rightH)
            Page.CanvasSize = UDim2.new(0, 0, 0, maxH + 40)
            
            -- Adjust container sizes for hit detection
            LeftColumn.Size = UDim2.new(0.48, 0, 0, leftH)
            RightColumn.Size = UDim2.new(0.48, 0, 0, rightH)
        end
        ...
        Tab.PageInstance = LeftColumn -- Default for raw appending
        ...
        function Tab:AddSection(sectionName, side)
            ...
            local SectionFrame = Utility:Create("Frame", {
                Parent = parentColumn, 
                BackgroundColor3 = Netro65UI.Theme.Secondary, 
                Size = UDim2.new(1, 0, 0, 0), -- Width is relative to column
                BorderSizePixel = 0, 
                BackgroundTransparency = 0.5
            }, {
                Utility:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
            ...
            ItemContainer.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
                SectionFrame.Size = UDim2.new(
                    1, 0, 
                    0, ItemContainer.UIListLayout.AbsoluteContentSize.Y + 55
                ) 
            end)

            --// Function to Move Section Side
            function Section:SetSide(newSide)
                if newSide == "Right" then
                    SectionFrame.Parent = RightColumn
                else
    ...
    local UISettings = SettingsTab:AddSection("UI Effects", "Left")
