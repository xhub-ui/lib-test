--[[
    NetroUI - UI Library 
    Status: (v5.3.3 Revised)
    Updates: Integrated new Assets, Icon System, and Enhanced Config
]]

local NetroUI = {}
NetroUI.Flags = {}
NetroUI.Setters = {}
NetroUI.ThemeObjects = {}
NetroUI.Keybinds = {}
NetroUI.ConfigFolder = "NetroUI"
NetroUI.ConfigFileSub = "settings"
NetroUI.CurrentConfigFile = "default.json"
NetroUI.Version = "5.3.3" 
NetroUI.IsVIP = false
...
local assets = {
    interFont = Enum.Font.GothamMedium, -- Fallback mapping
...
NetroUI.Theme = {
    ...
    FontMain    = assets.interFont, -- Using variable
...
function NetroUI:ToggleRainbowMode(enabled, speed)
    NetroUI.RainbowMode.Enabled = enabled
    NetroUI.RainbowMode.Speed = speed or 1
    
    if NetroUI.RainbowMode.Connection then
        NetroUI.RainbowMode.Connection:Disconnect()
        NetroUI.RainbowMode.Connection = nil
    end
    
    if enabled then
        NetroUI.RainbowMode.Connection = RunService.Heartbeat:Connect(function(delta)
            NetroUI.RainbowMode.Hue = (NetroUI.RainbowMode.Hue + delta * (NetroUI.RainbowMode.Speed * 0.1)) % 1
            local rainbowColor = Color3.fromHSV(NetroUI.RainbowMode.Hue, 0.8, 1)
            
            -- Update all accent elements
            for _, obj in pairs(NetroUI.ThemeObjects) do
...
function NetroUI:SetTheme(presetName)
    local preset = NetroUI.Presets[presetName] or NetroUI.Presets.Default
    NetroUI.Theme.Main = preset.Main
    NetroUI.Theme.Accent = preset.Accent
    
    if NetroUI.RainbowMode.Enabled then
        NetroUI:ToggleRainbowMode(false)
    end
    
    for _, obj in pairs(NetroUI.ThemeObjects) do
        if obj.Instance and obj.Instance.Parent then
            if obj.Type == "Main" then 
                obj.Instance.BackgroundColor3 = NetroUI.Theme.Main 
            end
            if obj.Type == "Secondary" then 
                obj.Instance.BackgroundColor3 = NetroUI.Theme.Secondary 
            end
            if obj.Type == "Accent" then 
                if obj.Property == "TextColor3" then 
                    obj.Instance.TextColor3 = NetroUI.Theme.Accent 
                elseif obj.Property == "ImageColor3" then
                    obj.Instance.ImageColor3 = NetroUI.Theme.Accent
                elseif obj.Property == "BackgroundColor3" then
                    obj.Instance.BackgroundColor3 = NetroUI.Theme.Accent
                elseif obj.Property == "ScrollBarImageColor3" then
                    obj.Instance.ScrollBarImageColor3 = NetroUI.Theme.Accent
                elseif obj.Property == "Color" then
                    obj.Instance.Color = NetroUI.Theme.Accent
                end
            end
        end
    end
...
--// Watermark System
local Watermark = nil
function NetroUI:CreateWatermark(text)
    ...
    UIStroke.Color = NetroUI.Theme.Accent
    UIStroke.Thickness = 1
    UIStroke.Parent = Watermark
    table.insert(NetroUI.ThemeObjects, {Instance = UIStroke, Type = "Accent", Property = "Color"})
    
    local Label = Instance.new("TextLabel")
    Label.Text = text or "NetroUI V5.3.3 | " .. LocalPlayer.Name
    Label.Font = NetroUI.Theme.FontBold
    Label.TextSize = 12
...
function Utility:AddToolTip(element, text)
    ...
    element.MouseEnter:Connect(function()
        ...
        tooltip = Utility:Create("Frame", {
            Name = "Tooltip",
            BackgroundColor3 = NetroUI.Theme.Main,
            BorderSizePixel = 0,
            Size = UDim2.new(0, 0, 0, 0),
            ...
            Utility:Create("UIStroke", {Color = NetroUI.Theme.Accent, Thickness = 1}),
            Utility:Create("TextLabel", {
                Text = text,
                Font = NetroUI.Theme.FontMain,
                TextSize = 12,
                TextColor3 = NetroUI.Theme.Text,
                BackgroundTransparency = 1,
        ...
        -- Calculate text size
        local textSize = TextService:GetTextSize(text, 12, NetroUI.Theme.FontMain, Vector2.new(200, math.huge))
...
--// Keybind System
function NetroUI:CreateKeybind(name, defaultKey, callback)
    local keybind = {
        Name = name,
        Key = defaultKey,
        Callback = callback,
...
function NetroUI:UpdateKeybind(name, newKey)
    if NetroUI.Keybinds[name] then
        NetroUI.Keybinds[name].Key = newKey
    end
end

function NetroUI:RemoveKeybind(name)
    if NetroUI.Keybinds[name] and NetroUI.Keybinds[name].Connection then
        NetroUI.Keybinds[name].Connection:Disconnect()
        NetroUI.Keybinds[name] = nil
    end
end
...
function Utility:ColorPicker(defaultColor, callback)
    ...
    local ColorFrame = Utility:Create("Frame", {
        ...
        Utility:Create("UIStroke", {Color = NetroUI.Theme.Accent, Thickness = 2}),
        Utility:Create("TextLabel", {
            Text = "Color Picker",
            Font = NetroUI.Theme.FontBold,
            TextSize = 16,
        ...
        -- Use ColorWheel Asset if desired, otherwise simple RGB sliders
        Utility:Create("ImageLabel", {
            Image = assets.colorWheel, -- Using asset
            BackgroundTransparency = 1,
            Size = UDim2.new(0, 100, 0, 100),
            Position = UDim2.new(0.5, -50, 0, 40),
            Visible = true
        })
    })
    ...
    local Preview = Utility:Create("Frame", {
        ...
        Utility:Create("UICorner", {CornerRadius = UDim.new(0, 6)}),
        Utility:Create("UIStroke", {Color = NetroUI.Theme.Outline, Thickness = 1})
    })
    ...
    for i = 1, 3 do
        ...
        local valueBox = Utility:Create("TextBox", {
            ...
            TextSize = 12,
            TextColor3 = NetroUI.Theme.Text,
            BackgroundColor3 = NetroUI.Theme.Secondary,
            Size = UDim2.new(0, 40, 0, 20),
            Position = UDim2.new(0.8, 0, 0, yPos)
        }, {
            Utility:Create("UICorner", {CornerRadius = UDim.new(0, 4)})
        })
        ...
        local track = Utility:Create("Frame", {
            ...
            Position = UDim2.new(0.2, 0, 0, yPos + 7)
        }, {
            Utility:Create("UICorner", {CornerRadius = UDim.new(1, 0)})
        })
    ...
    local CloseBtn = Utility:Create("TextButton", {
        ...
        TextSize = 14,
        TextColor3 = NetroUI.Theme.Text,
        BackgroundColor3 = NetroUI.Theme.Accent,
        Size = UDim2.new(0.8, 0, 0, 30),
...
--// Notification System (Reduced Size)
...
function NetroUI:Notify(props)
    local title = props.Title or "Notification"
    local content = props.Content or "Information"
    local duration = props.Duration or 4
    local typeColor = NetroUI.Theme.Accent
    
    if props.Type == "Success" then typeColor = NetroUI.Theme.Success 
    elseif props.Type == "Warning" then typeColor = NetroUI.Theme.Warning 
    elseif props.Type == "Error" then typeColor = NetroUI.Theme.Error end
    ...
    local NotifFrame = Utility:Create("Frame", {
        ...
        Utility:Create("TextLabel", {
            ...
            Font = NetroUI.Theme.FontMain, 
            TextSize = 11, 
            TextColor3 = NetroUI.Theme.TextDark, 
            BackgroundTransparency = 1, 
...
--// Config System Functions (Updated Folder Structure)
function NetroUI:GetConfigs(isUniversal)
    local configs = {}
    local path = NetroUI.ConfigFolder .. "/" .. NetroUI.ConfigFileSub .. "/" .. (isUniversal and "Universal" or tostring(game.PlaceId))
    
    if not isfolder(NetroUI.ConfigFolder) then
        if makefolder then
            makefolder(NetroUI.ConfigFolder)
            makefolder(NetroUI.ConfigFolder .. "/" .. NetroUI.ConfigFileSub)
        else
...
function NetroUI:DeleteConfig(name, isUniversal)
    local path = NetroUI.ConfigFolder .. "/" .. NetroUI.ConfigFileSub .. "/" .. (isUniversal and "Universal" or tostring(game.PlaceId)) .. "/" .. name .. ".json"
...
function NetroUI:SaveConfig(name, isUniversal)
    local folder = NetroUI.ConfigFolder
    if not isfolder(folder) then 
        if makefolder then
            makefolder(folder)
            makefolder(folder .. "/" .. NetroUI.ConfigFileSub)
        else
            NetroUI:Notify({
                Title = "Save Failed", 
    ...
    local path = folder .. "/" .. NetroUI.ConfigFileSub .. "/" .. (isUniversal and "Universal" or tostring(game.PlaceId))
    if not isfolder(path) then 
        if makefolder then
            makefolder(path)
        end
    ...
    local keybindsData = {}
    for name, kb in pairs(NetroUI.Keybinds) do
        keybindsData[name] = {
            Key = kb.Key,
            Name = kb.Name
        }
    ...
    local saveData = {
        ...
        Theme = {
            r = NetroUI.Theme.Accent.R,
            g = NetroUI.Theme.Accent.G,
            b = NetroUI.Theme.Accent.B
        }
    }
...
function NetroUI:LoadConfig(name, isUniversal)
    local path = NetroUI.ConfigFolder .. "/" .. NetroUI.ConfigFileSub .. "/" .. (isUniversal and "Universal" or tostring(game.PlaceId)) .. "/" .. name .. ".json"
    if not isfile(path) then 
        NetroUI:Notify({
            Title = "Error", 
    ...
    if success and content then
        ...
        if success2 and data then
            ...
            for flag, value in pairs(data.Flags or {}) do
                NetroUI.Flags[flag] = value
                if NetroUI.Setters[flag] then
                    NetroUI.Setters[flag](value)
                end
            end
            ...
            for name, keybindData in pairs(data.Keybinds or {}) do
                if keybindData.Key then
                    NetroUI:UpdateKeybind(name, keybindData.Key)
                end
            end
            
            -- Load theme if present
            if data.Theme then
                NetroUI.Theme.Accent = Color3.new(data.Theme.r, data.Theme.g, data.Theme.b)
                NetroUI:SetTheme("Default") -- Refresh theme
            end
...
--// Key System
function NetroUI:TriggerKeySystem(settings, onComplete)
    local KeySettings = {
    ...
    local KeyFrame = Utility:Create("Frame", {
        ...
        Utility:Create("UICorner", {CornerRadius = UDim.new(0, 8)}),
        Utility:Create("UIStroke", {Color = NetroUI.Theme.Outline, Thickness = 2}),
        Utility:Create("TextLabel", {
        ...
        Utility:Create("TextButton", {
            ...
            Font = NetroUI.Theme.FontBold, 
            TextSize = 16, 
            TextColor3 = NetroUI.Theme.Error, 
            BackgroundTransparency = 1, 
    ...
    local InputBox = Utility:Create("TextBox", {
        ...
        PlaceholderColor3 = NetroUI.Theme.TextDark,
        Text = "", 
    ...
    local ConfirmBtn = Utility:Create("TextButton", {
        ...
        Font = NetroUI.Theme.FontBold, 
        TextSize = 14, 
        TextColor3 = NetroUI.Theme.Main, 
        BackgroundColor3 = NetroUI.Theme.Success, 
    ...
    spawn(function()
        ...
        if isUserVIP then
            ...
            if useKeySystem then
                ...
                wait(1)
                NetroUI:TriggerKeySystem(config.KeySettings, finalCallback)
            else
                finalCallback()
            end
        end
...
function NetroUI:CreateWindow(props)
    local WindowObj = {}
    props = props or {}
    
    local windowWidth = props.Width or 610
    local windowHeight = props.Height or 380
    local titleText = props.Title or "NetroUI"
    ...
    NetroUI:CreateWatermark("NetroUI V5.3.3 | " .. LocalPlayer.Name)
    ...
    local MainFrame = Utility:Create("Frame", {
        ...
        Utility:Create("UICorner", {CornerRadius = UDim.new(0, 6)}), 
        Utility:Create("UIStroke", {Color = NetroUI.Theme.Outline, Thickness = 1.5})
    })
    
    table.insert(NetroUI.ThemeObjects, {
        Instance = MainFrame, 
        Type = "Main"
    })
    ...
    local BubbleButton = Utility:Create("TextButton", {
        ...
        BackgroundColor3 = bubbleConfig.Color or NetroUI.Theme.Accent,
        Visible = false,
        Text = "",
        AutoButtonColor = false,
        ZIndex = 5000
    ...
    elseif bubbleConfig.Type == "Text" or bubbleConfig.Type == "Emoji" then
        ...
        table.insert(NetroUI.ThemeObjects, {Instance = BubbleButton, Type = "Accent", Property = "BackgroundColor3"})
    ...
    local Header = Utility:Create("Frame", {
        ...
        Utility:Create("TextLabel", {
            Text = "v"..NetroUI.Version, 
            Font = NetroUI.Theme.FontMain, 
            TextSize = 11, 
            TextColor3 = NetroUI.Theme.Accent, 
            BackgroundTransparency = 1, 
    ...
    CloseBtn.MouseButton1Click:Connect(function() 
        local dialog = Utility:Create("Frame", {
            ...
            Utility:Create("TextButton", {
                ...
                BackgroundColor3 = NetroUI.Theme.Error,
                Font = NetroUI.Theme.FontBold,
                TextSize = 14,
        ...
        dialog.YesBtn.MouseButton1Click:Connect(function()
            dialog:Destroy()
            Acrylic:Disable() 
        ...
        dialog.NoBtn.MouseButton1Click:Connect(function()
            dialog:Destroy()
        end)
    end)
    ...
    local NavWidth = 115 
    --// UPDATED NAV CONTAINER: SCROLLING ENABLED //
    local NavContainer = Utility:Create("ScrollingFrame", {
        ...
        Position = UDim2.new(0, 0, 0, 10), 
        ScrollBarThickness = 2, -- Visible but thin for usability
        ScrollBarImageColor3 = NetroUI.Theme.Accent, -- Matches Theme
        CanvasSize = UDim2.new(0,0,0,0), 
        AutomaticCanvasSize = Enum.AutomaticSize.Y, -- Enables Auto-Scroll
        ClipsDescendants = true
    }, {
        Utility:Create("UIListLayout", {
            SortOrder = Enum.SortOrder.LayoutOrder, 
    ...
    Utility:Create("Frame", {
        Parent = ContentContainer, 
        BackgroundColor3 = NetroUI.Theme.Outline, 
    ...
    local ProfileCard = Utility:Create("Frame", {
        ...
        Utility:Create("UIStroke", {Color = NetroUI.Theme.Outline, Thickness = 1}), 
        Utility:Create("ImageLabel", {
            Image = assets.userInfoBlurred, -- Use asset
            ImageColor3 = NetroUI.Theme.Accent,
            Size = UDim2.new(1,0,1,0),
            BackgroundTransparency = 1,
            ZIndex = 0
        }, {Utility:Create("UICorner", {CornerRadius = UDim.new(0, 6)})}),
        ...
        Utility:Create("TextLabel", {
            Text = NetroUI.IsVIP and "VIP User" or "Free User", 
            Font = NetroUI.Theme.FontMain, 
            TextSize = 10, 
            TextColor3 = NetroUI.Theme.Accent, 
            BackgroundTransparency = 1, 
    ...
    function WindowObj:AddTab(name, iconId, layoutOrder)
        TabCount = TabCount + 1
        local currentOrder = layoutOrder or TabCount
        
        local Tab = {}
        local iconInfo = gl(iconId or "")
        ...
        local TabBtn = Utility:Create("TextButton", {
            ...
            Utility:Create("UIPadding", {PaddingLeft = UDim.new(0, 30)}), 
            ...
            Utility:Create("ImageLabel", {
                ...
                ImageColor3 = NetroUI.Theme.TextDark
            })
        })
        ...
        local Page = Utility:Create("ScrollingFrame", {
            ...
            ScrollBarImageColor3 = NetroUI.Theme.Accent, 
        ...
        TabBtn.MouseButton1Click:Connect(function() 
            ...
            for _, t in pairs(tabs) do 
                ...
                Utility:Tween(t.Btn.Icon, {0.2}, {ImageColor3 = NetroUI.Theme.TextDark})
            ...
            Utility:Tween(TabBtn.Icon, {0.2}, {ImageColor3 = NetroUI.Theme.Text})
            Page.Visible = true 
        end)
        
        if #tabs == 0 then 
            TabBtn.BackgroundTransparency = 0.92 
            TabBtn.BackgroundColor3 = NetroUI.Theme.Accent 
            TabBtn.TextColor3 = NetroUI.Theme.Text 
            TabBtn.Icon.ImageColor3 = NetroUI.Theme.Text
            Page.Visible = true 
        end
        ...
        function Tab:AddSection(sectionName, side)
            ...
            function Section:AddButton(bProps)
                ...
                Button.MouseEnter:Connect(function() 
                    Utility:Tween(Button, {0.2}, {
                        BackgroundColor3 = NetroUI.Theme.Hover
                    }) 
                end)
            ...
            function Section:AddToggle(tProps)
                ...
                if flag and NetroUI.Flags[flag] ~= nil then 
                    state = NetroUI.Flags[flag] 
                end
                ...
                -- Using Asset Logic
                local Switch = Utility:Create("ImageLabel", {
                    Parent = ToggleFrame, 
                    Image = assets.toggleBackground,
                    BackgroundTransparency = 1,
                    ImageColor3 = state and NetroUI.Theme.Accent or NetroUI.Theme.Main, 
                ...
                local Knob = Utility:Create("ImageLabel", {
                    Parent = Switch, 
                ...
                local function Update(val) 
                    ...
                    Utility:Tween(Switch, {0.2}, {
                        ImageColor3 = state and NetroUI.Theme.Accent or NetroUI.Theme.Outline
                    }) 
                    if flag then 
                        NetroUI.Flags[flag] = state 
                    end 
                    callback(state) 
                end
                ...
                if flag then 
                    NetroUI.Flags[flag] = state 
                    NetroUI.Setters[flag] = Update
                end 
                
                Update(state) -- Init visual state
            ...
            function Section:AddSlider(sProps)
                ...
                local Track = Utility:Create("ImageLabel", {
                    Parent = SliderFrame, 
                    Image = assets.sliderbar,
                    BackgroundTransparency = 1,
                    ImageColor3 = NetroUI.Theme.Outline, 
                ...
                local Fill = Utility:Create("ImageLabel", {
                    Parent = Track, 
                    Image = assets.sliderbar,
                    BackgroundTransparency = 1,
                    ImageColor3 = NetroUI.Theme.Accent, 
                ...
                local Head = Utility:Create("ImageLabel", {
                    Parent = Track,
                ...
                if flag then 
                    NetroUI.Flags[flag] = value 
                    NetroUI.Setters[flag] = UpdateSlider
                end 
            ...
            function Section:AddDropdown(dProps)
                ...
                local HeaderBtn = Utility:Create("TextButton", {
                    ...
                    Utility:Create("ImageLabel", {
                        Image = assets.dropdown, -- Asset used
                        BackgroundTransparency = 1, 
                        Size = UDim2.new(0, 20, 0, 20), 
                ...
                if flag then 
                    NetroUI.Flags[flag] = default
                    NetroUI.Setters[flag] = function(v) 
            ...
            function Section:AddKeybind(kbProps)
                ...
                KeyButton.MouseButton1Click:Connect(function()
                    ...
                    if listening then
                        KeyButton.Text = "..."
                        KeyButton.BackgroundColor3 = NetroUI.Theme.Accent
                        ...
                        connection = UserInputService.InputBegan:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.Keyboard then
                                ...
                                KeyButton.BackgroundColor3 = NetroUI.Theme.Main
                                listening = false
                                
                                if flag then
                                    NetroUI.Flags[flag] = currentKey
                                end
                ...
                if flag then
                    NetroUI.Flags[flag] = currentKey
                    NetroUI.Setters[flag] = function(key)
                        currentKey = key
            ...
            function Section:AddColorPicker(cpProps)
                ...
                if flag then
                    NetroUI.Flags[flag] = currentColor
                    NetroUI.Setters[flag] = function(color)
                        currentColor = color
    ...
    local function GeneratePlayerInfo(parentPage)
        ...
        table.insert(NetroUI.ThemeObjects, {Instance = ProfileCard, Type = "Secondary"})
        ...
        addTextLine("Username: @" .. LocalPlayer.Name, 14, NetroUI.Theme.FontBold, Color3.new(1,1,1), 1, InfoContainer)
        ...
        CopyBtn.MouseButton1Click:Connect(function()
            if setclipboard then
                ...
                CopyBtn.Text = "Copied"
                CopyBtn.BackgroundColor3 = NetroUI.Theme.Success
                task.wait(1)
        ...
        local function createMiniCard(title, valueText, valueColor, isTimeCard)
            ...
            table.insert(NetroUI.ThemeObjects, {Instance = card, Type = "Secondary"})
            ...
            if isTimeCard then
                table.insert(NetroUI.ThemeObjects, {Instance = v, Type = "Accent", Property = "TextColor3"})
        ...
        local statusVal = NetroUI.IsVIP and "VIP MEMBER" or "FREE USER"
        local statusCol = NetroUI.IsVIP and Color3.fromRGB(255, 215, 0) or Color3.fromRGB(200, 200, 200)
        createMiniCard("ACCOUNT", statusVal, statusCol, false)

        local timeLabel = createMiniCard("LOCAL TIME", "00:00:00", NetroUI.Theme.Accent, true)
        ...
        table.insert(NetroUI.ThemeObjects, {Instance = MarketingCard, Type = "Secondary"})
        local mStroke = MarketingCard:FindFirstChild("UIStroke")
        if mStroke then
            table.insert(NetroUI.ThemeObjects, {Instance = mStroke, Type = "Accent", Property = "Color"})
        end
        ...
        table.insert(NetroUI.ThemeObjects, {Instance = UpgradeBtn, Type = "Accent", Property = "BackgroundColor3"})
        ...
        UpgradeBtn.MouseButton1Click:Connect(function()
            if setclipboard then
                ...
                NetroUI:Notify({Title="VIP Upgrade", Content="Purchase link copied!", Type="Success", Duration=3})
    ...
    local PlayersInfoTab = WindowObj:AddTab("Players Info", assets.globe, -9999)
    GeneratePlayerInfo(PlayersInfoTab.PageInstance)

    --// AUTO-GENERATE SETTINGS TAB
    local SettingsTab = WindowObj:AddTab("UI Settings", assets.grid, 99999)
    ...
    ConfigSection:AddButton({
        ...
        Callback = function() 
            if ConfigName ~= "" then 
                NetroUI:SaveConfig(ConfigName, false) 
            else 
                NetroUI:Notify({
                    Title = "Error", 
    ...
    ConfigSection:AddDropdown({
        Name = "Load Config", 
        Items = NetroUI:GetConfigs(false), 
        Callback = function(v) 
            ConfigName = v
            NetroUI:LoadConfig(v, false) 
        end
    })
    ...
    ConfigSection:AddButton({
        ...
        Callback = function() 
            if ConfigName ~= "" then 
                NetroUI:DeleteConfig(ConfigName, false) 
            else 
                NetroUI:Notify({
                    Title = "Error", 
    ...
    for name, _ in pairs(NetroUI.Presets) do 
    ...
    ThemeSection:AddSlider({
        ...
        Callback = function(v)
            if NetroUI.RainbowMode.Enabled then
                NetroUI:ToggleRainbowMode(true, v)
            end
        end
    })
    ...
    NetroUI:CreateKeybind("ToggleUI", Enum.KeyCode.RightControl, function()
