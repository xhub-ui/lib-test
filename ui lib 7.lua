--[[
    NetroUI - UI Library (V5.0 - Revised & Optimized)
    Status: Enhanced Version with Layout Fixes & Bottom Settings
]]
...
function Netro65UI:ToggleAutoSave(enabled, interval)
    ...
    if enabled then
        AutoSave.Connection = RunService.Heartbeat:Connect(function()
            task.wait(interval)
            Netro65UI:SaveConfig("autosave", false)
        end)
    end
end
...
function Netro65UI:CreateServerInfoPanel()
    local panel = Utility:Create("Frame", {
        ...
        Position = UDim2.new(1, -210, 1, -160),
        ZIndex = 100
    }, {
        Utility:Create("UICorner", {CornerRadius = UDim.new(0, 8)}),
        Utility:Create("UIStroke", {Color = Netro65UI.Theme.Accent, Thickness = 1}),
    ...
    connection = RunService.Heartbeat:Connect(function()
        if not panel.Parent then connection:Disconnect() return end
        playerCount = #Players:GetPlayers()
        labels[1].Text = "Players: " .. playerCount .. "/" .. maxPlayers
        
        -- Simple FPS counter
        local fps = math.floor(1/RunService.RenderStepped:Wait())
        labels[4].Text = "FPS: " .. fps
    end)
...
function Netro65UI:CreateWindow(props)
    ...
    MinBtn.MouseButton1Click:Connect(function() 
        ...
        if IsMin then 
            Utility:Tween(MainFrame, {0.4}, {
                Size = UDim2.new(0, windowWidth, 0, 45)
            }) 
    ...
    local NavContainer = Utility:Create("ScrollingFrame", {
        ...
        BackgroundTransparency = 1, 
        Size = UDim2.new(0, NavWidth, 1, -65), 
        Position = UDim2.new(0, 0, 0, 10), 
        ...
        function Tab:AddSection(sectionName)
            ...
            ItemContainer.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function() 
                SectionFrame.Size = UDim2.new(
                    0.96, 0, 
                    0, ItemContainer.UIListLayout.AbsoluteContentSize.Y + 45
                ) 
            end)
    ...
    local SettingsTab = WindowObj:AddTab("Settings", 99999)
    ...
    ConfigSection:AddButton({
        ...
        Callback = function() 
            if ConfigName ~= "" then 
                Netro65UI:SaveConfig(ConfigName, UniversalConfig) 
            else 
                Netro65UI:Notify({
                    Title = "Error", 
    ...
    -- Keybinds Section
    local KeybindsSection = SettingsTab:AddSection("Keybinds")
    
    -- Register some default keybinds
    Netro65UI:CreateKeybind("ToggleUI", Enum.KeyCode.RightControl, function()
        MainFrame.Visible = not MainFrame.Visible
        if MainFrame.Visible and Acrylic.Active then Acrylic:Enable() else Acrylic:Disable() end
    end)
    
    Netro65UI:CreateKeybind("CommandBar", Enum.KeyCode.Slash, function()
        Netro65UI:ShowCommandBar()
    end)
